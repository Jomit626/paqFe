...
# 51 "MatchModel.hpp"
    for(int i = 0; i < 8; i++) StateMap<1 << (5+i)> smi;;
...
public:
    ...
    void predict_byte(uint8_t byte, Prob *pp, Context *pctx, size_t pstride, size_t ctxstride) {
        ...
        if(length > 0){
            for(int bpos = 0; bpos < 8; bpos++){
                ...
                matchCxts[bpos] = (((length - 1) * 2 + expectedBit) << bpos) | (byte >> (8 - bpos));
            }
        }
        ...
        for(int i = 0; i < 8; i++) { smi.predict(matchCxts[i], &pp[pstride]); smi.update(byte >> (7 - i));};
    ...
    void state2prob(uint16_t mc, Prob *pp){
